...
INSERT INTO categories (id, slug, title_ua, title_en, description_ua, description_en, "order") VALUES
  ...
  (
    ...
    'Підкачка',
    'Conditioning',
    'Силова підготовка і витривалість',
  ...
  (
    ...
    'Здоров''я',
    'Prehab',
    'Профілактика травм, розминка, відновлення',
    'Injury prevention, warm-up, recovery',
    4
  ),
  (
    ...
    'Координація',
    'Coordination',
    'Контроль тіла, виходи, переходи між елементами',
    'Body control, exits, transitions between elements',
    5
  );
...
-- ============================================================
-- EXERCISE CATEGORIES (radar chart tags)
...
WHERE id IN (
  '00000000-0000-0000-0004-000000000001', -- Розминка зап'ястків
  '00000000-0000-0000-0004-000000000027', -- Розминка зап'ястків і плечей
  '00000000-0000-0000-0004-000000000042', -- Розминка 5 хвилин
  '00000000-0000-0000-0004-000000000044'  -- Заминка і розтяжка плечей
);

-- conditioning: силові і кондиційні вправи
UPDATE exercises SET exercise_category_id = '00000000-0000-0000-0000-000000000003'
WHERE id IN (
  '00000000-0000-0000-0004-000000000004', -- Hollow body hold
  '00000000-0000-0000-0004-000000000005', -- Superman hold
  '00000000-0000-0000-0004-000000000007', -- Відтискання з широкою постановкою
  '00000000-0000-0000-0004-000000000009', -- Donkey kicks
  '00000000-0000-0000-0004-000000000013', -- Підйоми плечей в стійці
  '00000000-0000-0000-0004-000000000018', -- Протракція лопаток
  '00000000-0000-0000-0004-000000000019', -- Pike push-up
  '00000000-0000-0000-0004-000000000026'  -- Hollow body на одній нозі
);

-- coordination: виходи, переходи, контроль падіння
UPDATE exercises SET exercise_category_id = '00000000-0000-0000-0000-000000000005'
WHERE id IN (
  '00000000-0000-0000-0004-000000000039', -- Вихід через перекид
  '00000000-0000-0000-0004-000000000040', -- Пірует-вихід
  '00000000-0000-0000-0004-000000000038'  -- Відновлення після падіння
);

-- handstand: всі баланс/стійка вправи (решта)
