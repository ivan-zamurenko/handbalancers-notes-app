...
export default function ProgramCard({ program, locale }: Props) {
  ...
  return (
    <div style={{ border: '1px solid #1e1e1e', borderRadius: '8px', padding: '1rem', background: '#141414' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <h3 style={{ margin: 0, color: '#fff' }}>{title}</h3>
        <span style={{ fontSize: '0.75rem', padding: '2px 8px', borderRadius: '4px', background: program.is_free ? 'rgba(57, 230, 0, 0.1)' : 'rgba(255, 200, 0, 0.1)', color: program.is_free ? '#39e600' : '#f5a623' }}>
          {program.is_free ? t('free') : t('paid')}
        </span>
      </div>
      {description && <p style={{ color: '#888', margin: '0.5rem 0' }}>{description}</p>}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.5rem' }}>
        <span style={{ fontSize: '0.875rem', color: '#555' }}>{levelLabel}</span>
        <Link href={`/programs/${program.slug}`} style={{ color: '#2979ff' }}>{t('view')} →</Link>
      </div>
    </div>
  )
}
